    ...
    <script>
        ...
        function fallbackCopyTextToClipboard(text) {
            ...
            document.body.removeChild(textArea);
                                                ...
                                                <p style="margin: 0; font-size: 14px;">📱 Voucher details sent to ${result.phone_number || mpesaNumber}</p>
                                            </div>
                                        </div>

                                        <p style="font-size: 12px; margin-top: 10px; text-align: center; color: #6b7280;">Receipt: ${result.receipt || 'N/A'}</p>
                                    </div>
                                `;
                                ...
                                if (result.voucher_code && result.phone_number) {
                                    ...
                                    .then(smsResult => {
                                        ...
                                        if (!smsResult.success) {
                                            ...
                                            const smsErrorDiv = document.createElement('div');
                                            smsErrorDiv.style.cssText = 'background-color: #fef2f2; color: #dc2626; padding: 10px; border-radius: 6px; margin-top: 10px; text-align: center; font-size: 14px;';
                                            smsErrorDiv.innerHTML = '⚠️ Failed to send voucher via SMS. Please save the details above.';
                                            document.getElementById('payment-status').appendChild(smsErrorDiv);
                                        }
                                    }).catch(error => {
