...
// Set up logging
$log_file = '../logs/manual_payments.log';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($checkStmt === false) {
        log_payment("Error preparing check statement: " . $conn->error);
    ...
    if (empty($errors)) {
        try {
            ...
            if ($packageStmt === false) {
                throw new Exception("Error preparing package statement: " . $conn->error);
            }
            ...
            if ($result) {
                ...
                // Router integration disabled - vouchers generated without router communication
                ...
                if ($tableCheck && $tableCheck->num_rows > 0) {
                    ...
                    if ($voucherStmt === false) {
                        log_payment("Error preparing voucher insert: " . $conn->error);
                    } else {
                        ...
                        if ($voucherStmt->affected_rows > 0) {
                            ...
                            $mikrotikResult = addVoucherToMikrotik($voucher_code, $package_id, $reseller_id, $phone_number, $conn);
                            if ($mikrotikResult === true) {
                                log_payment("Voucher added to MikroTik: $voucher_code");
                            } else {
                                log_payment("Failed to add voucher to MikroTik: $mikrotikResult");
                            }
                        }
                    }
                } else {
                    // Add voucher to MikroTik even without vouchers table
                    $mikrotikResult = addVoucherToMikrotik($voucher_code, $package_id, $reseller_id, $phone_number, $conn);
                    if ($mikrotikResult === true) {
                        log_payment("Voucher added to MikroTik: $voucher_code");
                    } else {
